...
function UserInputs({ setResumeData, resumeData = {} }) {
  ...
  const handleAddResume= async ()=>{
    ...
    if(fullName && location && job && email && phone&& github&& linkedin&& degree && college && year && skills && summary){
     const resp = await saveResumeAPI(resumeData)
    if(resp.status == 201){
      alert("form submited successfully")
      setTimeout(()=>{
      navigate(`/resume/${resp.data.id}/view`)
      },2000)
    }else {
  ...
  const renderFormContent = (stepCount) => {
    ...
    switch (stepCount) {
      ...
      case 3: return (
        <div>
          <p>
            Our AI will generate Skills & Summary according to your job role. Once the form get submitted, user won't get the chance to update the resume details. If you want to proceed please click the <b onClick={generateSkills}>Generate AI Skill & Summary</b> button to submit.
          </p>
        </div>
      )
        ...
        {activeStep === steps.length ? (
          ...
          <React.Fragment>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
              ...
              {activeStep === steps.length - 1 ? <Button onClick={generateSkills}>Generate AI Skills and Summary</Button> : <Button onClick={handleNext} >Next</Button>}
